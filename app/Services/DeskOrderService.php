...
class DeskOrderService
{
    ...
    public function baseQueryForSession(array $deskUser, bool $includeClosed = false): Builder
    {
        ...
        $hidden = DeskAccess::hiddenRawStatuses($deskUser);
        if ($hidden !== []) {
            $query->where(function ($q) use ($hidden) {
                $q->whereNotIn('raw_status', $hidden)
    ...
    public function close(DeskOrderCache $cached, array $deskUser): array
    {
        ...
        try {
            DB::transaction(function () use ($adapter, $cached, $externalId, $crmId, &$calc) {
                ...
                // КП отдаёт свою расчётку; для CRM1 — из ответа LC или локальная формула
                if (is_array($closeResult) && ($closeResult['source'] ?? null) === 'kp') {
                    $calc = $closeResult;
                } elseif (is_array($closeResult) && isset($closeResult['master_percent'])) {
                    $calc = $closeResult;
                } else {
                    $calc = \App\Support\DeskMasterCalc::computeFromCache($cached);
                }
    ...
    public function assertCloseable(DeskOrderCache $cached): void
    {
        ...
        // Как в CRM Levelion: документы обязательны только при оплате >= 3000 ₽
        if ($paid >= 3000) {
            $contractDocs = array_filter($docs, fn ($d) => ($d['category'] ?? '') === 'contract');
            if (count($docs) < 1) {
                $errors[] = 'При сумме от 3000 ₽ загрузите хотя бы один документ';
            } elseif ($cached->connection?->type === 'crm2_http' && count($contractDocs) < 1) {
                $errors[] = 'При сумме от 3000 ₽ загрузите документ в «Договор/Чеки на услуги»';
            }
        }

...
@php
    ...
    $citiesMsText = $citiesFilterAll || $selectedCitiesCount === $citiesTotal
        ? 'Все города'
        : ($selectedCitiesCount ? ($selectedCitiesCount.' гор.') : 'Город');
@endphp

@if($citiesFilterLabel !== '')
        ...
        <table class="orders-sticky-table">
            <thead>
                ...
                <tr>
                    ...
                    <td>
                        @if(count($cities) > 0)
                        <input type="hidden" name="filter_cities" value="1">
                        <div class="multiselect multiselect-compact desk-cities-ms" data-name="cities" id="desk-cities-ms">
                            <div class="multiselect-selected" onclick="toggleMultiselect(this)">
                                ...
                                <div class="desk-cities-ms__list" id="desk-cities-ms-list">
                                    @foreach($cities as $c)
                                        ...
                                        <label data-city-label="{{ mb_strtolower($c['label'].' '.$c['name']) }}">
                                            <input type="checkbox" name="cities[]" value="{{ $cid }}"
                                                @checked(in_array($cid, $selectedCityIds, true))
                                                onchange="updateCitiesMs(this)">
                                            <span>{{ $c['label'] }}</span>

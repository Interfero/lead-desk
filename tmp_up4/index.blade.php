...
@if(count($cities) > 0)
<div class="desk-cities-filter card">
    <div class="desk-cities-filter__head">
        <div>
            <strong>Города</strong>
            <span class="muted" style="font-size:12px;margin-left:6px;">{{ $citiesFilterLabel }}</span>
        </div>
        <div class="desk-cities-filter__actions">
    ...
    <div class="desk-cities-chips" id="desk-cities-chips">
        @foreach($cities as $c)
            @php $cid = (int) $c['id']; @endphp
            <label class="desk-city-chip {{ in_array($cid, $selectedCityIds, true) ? 'is-on' : '' }}">
                <input type="checkbox" form="deskFiltersForm" name="cities[]" value="{{ $cid }}"
                    @checked(in_array($cid, $selectedCityIds, true))
                    class="desk-city-chip__input">
                <span>{{ $c['label'] }}</span>
    ...
    <p class="muted" style="margin:8px 0 0;font-size:12px;">
        Фильтр только для просмотра списка. Синхронизация и права доступа не меняются — заявки других городов не удаляются.
    </p>
</div>
@endif
        ...
        <table class="orders-sticky-table">
            <thead>
                ...
                <tr>
                    ...
                    <td>
                        <span class="muted" style="font-size:11px;">фильтр↑</span>
                    </td>
                    <td>
...
(function(){
    const chips=document.getElementById('desk-cities-chips');
    if(!chips) return;
    function syncChip(label){
        const on=!!label.querySelector('input')?.checked;
        label.classList.toggle('is-on', on);
    }
    chips.querySelectorAll('.desk-city-chip').forEach(label=>{
        syncChip(label);
        label.querySelector('input')?.addEventListener('change',()=>syncChip(label));
    });
    document.getElementById('desk-cities-all')?.addEventListener('click',()=>{
        chips.querySelectorAll('input[type=checkbox]').forEach(c=>{ c.checked=true; syncChip(c.closest('.desk-city-chip')); });
    });
    document.getElementById('desk-cities-none')?.addEventListener('click',()=>{
        chips.querySelectorAll('input[type=checkbox]').forEach(c=>{ c.checked=false; syncChip(c.closest('.desk-city-chip')); });
    });
})();
</script>
@endpush

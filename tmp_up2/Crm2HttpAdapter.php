...
class Crm2HttpAdapter implements CrmAdapter
{
    ...
    public function fetchOrders(array $filters = []): array
    {
        ...
        $orders = $this->parseOrdersHtml($body);
    ...
    public function fetchOrder(string $externalId): ?array
    {
        ...
        // город для списка берём из колонки КП при sync; карточка не должна затирать «Коломна (Рязань) / …»
    ...
    public function parseOrderDetailHtml(string $html, string $externalId): array
    {
        ...
        $cityName = null;
        // город с select на карточке (если есть), без эвристики по адресу
    ...
    public function updateOrder(string $externalId, array $data): void
    {
        ...
        if (! empty($data['comment'])) {
            ...
            $line = '['.$stamp.' desk] '.trim((string) $data['comment']);
            $prev = trim((string) ($payload['CustomerRequest[comments]'] ?? ''));
            $payload['CustomerRequest[comments]'] = $prev !== '' ? $prev."\n".$line : $line;
        }
    ...
    {
        $this->cookies ??= new CookieJar();
        $timeout = (int) ($this->connection->timeout ?: 20);

...
<div class="orders-top">
    ...
    <div style="display:flex;gap:10px;flex-wrap:wrap;align-items:center;">
        @foreach($connections as $crm)
            ...
            @endphp
            <form method="POST" action="{{ route('desk.sync', $crm->id) }}" style="display:inline-flex;gap:6px;align-items:center;">
                @csrf
                <span class="dot {{ $crm->status === 'active' ? 'dot-ok' : ($crm->status === 'error' ? 'dot-err' : 'dot-warn') }}" title="{{ $crm->last_error ?: ($crm->last_sync_at ?: 'нет sync') }}"></span>
                <span style="font-size:14px;">{{ $crm->name }}</span>
        ...
        <table class="orders-sticky-table">
            <thead>
                ...
                <tr>
                    ...
                    <td>
                        <select name="city" class="form-input table-filter" onchange="this.form.submit()">
                            <option value="">Выбрать город</option>
                            @foreach($cities as $c)
                                <option value="{{ $c['id'] }}" @selected((string)request('city')===(string)$c['id'] || (string)request('city_id')===(string)$c['id'])>{{ $c['name'] }}</option>
                            @endforeach
                        </select>
                    </td>
                    <td>
...
<div class="desk-order-shell" id="desk-order-shell">
    <div class="desk-order-h">
        ...
        <div style="display:flex;gap:8px;flex-wrap:wrap;">
            <button type="button" class="btn btn-sm" id="desk-btn-copy">Копировать</button>
            <button type="button" class="btn btn-sm" id="desk-btn-save">Сохранить</button>
            <button type="button" class="btn btn-primary btn-sm" id="desk-btn-close-order">Провести</button>
...
(function(){
    ...
    function updNet(){
        ...
        if(el) el.textContent=(paid-parts).toLocaleString('ru-RU');
        previewCalc();
    }
    ...
    function docsHtml(docs, cats, orderId, readOnly){
        ...
        return Object.entries(cats||{}).map(([code,label])=>{
            ...
            const note = code==='parts_photos' ? `<div class="desk-doc-note">Запчасть должна быть на фоне чека</div>` : '';
            const drop = readOnly
                ? `<div class="muted" style="font-size:12px;margin-bottom:8px;">Только просмотр</div>`
                : `<div class="desk-dropzone" data-upload="${esc(code)}">Перетащите фото сюда или нажмите для выбора</div>
                   <input type="file" accept=".jpg,.jpeg,.png,.gif,image/*" multiple hidden data-file="${esc(code)}">`;
            return `<div class="desk-doc-panel" data-cat="${esc(code)}">
                <h4>${esc(label)}</h4>
                ${drop}
                <div class="desk-files" data-files="${esc(code)}">${list}</div>${note}
            </div>`;
        }).join('');
    }
    ...
    function render(p){
        ...
        document.getElementById('desk-btn-save').style.display=readOnly?'none':'';
        ...
        body.innerHTML=`
        <div class="desk-order-page">
            <div class="desk-order-layout">
                <div class="desk-order-main">
                    <h3 class="desk-order-section-title">Информация о заказе</h3>
                    <div class="desk-kv-grid">
                        <div class="desk-kv"><div class="desk-form-label">Клиент</div><div class="desk-form-value">${esc(o.client_name||'—')}</div></div>
                        <div class="desk-kv"><div class="desk-form-label">Телефон</div><div class="desk-form-value">${esc(o.phone||'—')}</div></div>
                        <div class="desk-kv desk-kv--full"><div class="desk-form-label">Адрес</div><div class="desk-form-value">${esc(o.address||'—')}</div></div>
                        <div class="desk-kv"><div class="desk-form-label">Город</div><div class="desk-form-value">${esc(o.city_name||'—')}</div></div>
                        <div class="desk-kv"><div class="desk-form-label">Тип</div><div class="desk-form-value">${esc(typeLabel)}</div></div>
                        <div class="desk-kv"><div class="desk-form-label">Время заявки</div><div class="desk-form-value desk-call-at ${callAtCls}">${esc(callAt||'—')}</div></div>
                        <div class="desk-kv"><div class="desk-form-label">Непрофиль</div><div class="desk-form-value">${o.is_noncore?'Да':'Нет'}</div></div>
                        <div class="desk-kv desk-kv--full">
                            <div class="desk-form-label">История / комментарии</div>
                            <pre class="desk-comments-box">${esc(o.comments||o.description||'—')}</pre>
                        </div>
                    </div>
                </div>
                <aside class="desk-order-sidebar">
                    <div class="desk-sidebar-id">№ ${esc(o.external_id)}</div>
                    <div class="desk-sidebar-meta">${esc(o.connection?.name||'')} · ${esc(statusLabel)}</div>
                    <div class="desk-form-field">
                        <label class="desk-form-label" for="desk-f-status">Статус</label>
                        <select id="desk-f-status" class="form-control" ${readOnly?'disabled':''}>${statusOpts}</select>
                    </div>
                    <div class="desk-form-field">
                        <label class="desk-form-label" for="desk-f-master">Мастер</label>
                        <select id="desk-f-master" class="form-control" ${readOnly?'disabled':''}>${masterOpts}</select>
                    </div>
                    <div class="desk-amounts-row">
                        <div class="desk-form-field" style="margin:0">
                            <label class="desk-form-label" for="desk-f-paid">Оплачено</label>
                            <input id="desk-f-paid" type="number" min="0" class="form-control" value="${esc(paid)}" ${readOnly?'disabled':''}>
                        </div>
                        <div class="desk-form-field" style="margin:0">
                            <label class="desk-form-label" for="desk-f-parts">Комплектующие</label>
                            <input id="desk-f-parts" type="number" min="0" class="form-control" value="${esc(parts)}" ${readOnly?'disabled':''}>
                        </div>
                    </div>
                    <div class="desk-net-box">
                        <div class="desk-form-label">Чистыми</div>
                        <div class="desk-net" id="desk-f-net">0</div>
                    </div>
                    ${isKp ? `
                    <div class="desk-form-field">
                        <label class="desk-form-label" for="desk-f-prepayment">Предоплата</label>
                        <input id="desk-f-prepayment" type="number" min="0" class="form-control" value="${esc(prep)}" ${readOnly?'disabled':''}>
                    </div>
                    <div class="desk-amounts-row">
                        <div class="desk-form-field" style="margin:0">
                            <label class="desk-form-label" for="desk-f-bso">БСО</label>
                            <select id="desk-f-bso" class="form-control" ${readOnly?'disabled':''}>
                        ...
                        <div class="desk-form-field" style="margin:0">
                            <label class="desk-form-label" for="desk-f-zip">ЗПЧ</label>
                            <select id="desk-f-zip" class="form-control" ${readOnly?'disabled':''}>
                                <option value="">Выберите</option>
                    ...
                    <div class="desk-form-field">
                        <label class="desk-form-label" for="desk-f-comment">Комментарий к сохранению</label>
                        <textarea id="desk-f-comment" class="form-control" rows="3" ${readOnly?'disabled':''} placeholder="Добавится к истории"></textarea>
                    </div>
                </aside>
            </div>

            <section class="desk-order-documents">
                <div class="desk-docs-heading">
                    <span>Документы заказа</span>
                    <span class="muted" style="font-size:12px;font-weight:500;">${docsCount} файл(ов)</span>
                </div>
                <div class="desk-docs-grid">${docsHtml(o.documents, p.doc_categories, o.id, readOnly)}</div>
            </section>

            <section class="desk-order-history">
                <h3 class="desk-order-section-title">История заказов клиента</h3>
                ${hist.length?`<p class="muted" style="margin:0 0 8px;font-size:12px;">Показаны записи <strong style="color:var(--text)">1–${hist.length}</strong> из <strong style="color:var(--text)">${hist.length}</strong>.</p>`:''}
                <div class="desk-hist-scroll">
                    <table class="desk-hist-table">
                        <thead>
                            <tr>
                                <th>ID</th><th>Тип</th><th>Прозвон</th>
                                <th>Дата принятия заявки</th><th>Дата исполнения</th>
                                <th>Статус</th><th>Мастер</th><th>Сумма заявки, руб.</th>
                                <th>Кем создана</th><th>РК</th>
                            </tr>
                        </thead>
            ...
            <section class="desk-order-master" id="desk-master-section" style="${lastCalc?'':'display:none'}">
                <h3 class="desk-order-section-title">Расчётка мастера</h3>
